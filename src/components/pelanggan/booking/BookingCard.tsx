...
export default function BookingCard({
    ...
    const canReschedule = () => {
        ...
        if (![1, 2].includes(statusId)) return false;
    ...
    // 1. Link WA untuk Maintenance (ID 5)
    ...
    // 3. Link WA untuk Refund Manual (ID 3)
    ...
    const showCancel = [1, 2].includes(statusId) && !isHistory;
    const showReschedule = canReschedule();
    const showPrint = statusId === 2 || statusId === 4;
    const hasFooterActions = showCancel || showReschedule || showPrint;
    ...
    return (
        <Card className={`overflow-hidden border border-gray-200 border-l-4 ${borderClass} rounded-xl shadow-sm hover:shadow-md transition-all duration-300`}>
            <CardContent className="p-5 sm:p-6">
                <div className="flex flex-col md:flex-row justify-between gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        {statusId === 3 && (Number(booking.jumlah_bayar) > 0 || booking.bukti_pembayaran) && (
                    ...
                    <div className="flex md:flex-col justify-between items-center md:items-end bg-slate-50/80 rounded-xl p-4 sm:p-5 border border-slate-100 md:min-w-[200px] h-fit">
                        <div className="text-left md:text-right w-full">
                            ...
                            <p className={`text-lg sm:text-2xl font-black tracking-tight ${(statusId === 5 || statusId === 3) ? 'text-gray-400 line-through' : 'text-[#D93F21]'}`}>
            ...
            {(statusId !== 5 && statusId !== 3 && hasFooterActions) && (
