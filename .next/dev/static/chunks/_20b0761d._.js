...
function CartProvider({ children }) {
    ...
    const [pendingCart, setPendingCart] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(null);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "CartProvider.useEffect": ()=>{
            const controller = new AbortController();
            const loadAndValidateCart = {
                "CartProvider.useEffect.loadAndValidateCart": async ()=>{
                    const savedCart = localStorage.getItem('maindihatta_cart');
                    if (!savedCart) return;
                    let parsed;
                    try {
                        ...
                        localStorage.removeItem('maindihatta_cart');
                        return;
                    }
                    // Sanitize defaults
                    const sanitized = parsed.map({
                        "CartProvider.useEffect.loadAndValidateCart.sanitized": (it)=>({
                                ...it,
                                qty: typeof it.qty === 'number' && it.qty > 0 ? it.qty : 1,
                                catatan: typeof it.catatan === 'string' ? it.catatan : ''
                            })
                    }["CartProvider.useEffect.loadAndValidateCart.sanitized"]);
                    // Validate against current product list to avoid showing removed/unavailable items
                    try {
                        // Lazy-import axios to avoid circular deps in some setups
                        const axios = (await __turbopack_context__.A("[project]/src/lib/axios.ts [app-client] (ecmascript, async loader)")).default;
                        const res = await axios.get('/api/produkRestos', {
                            signal: controller.signal
                        });
                        const products = res.data || [];
                        const productMap = new Map(products.map({
                            "CartProvider.useEffect.loadAndValidateCart": (p)=>[
                                    p.id,
                                    p
                                ]
                        }["CartProvider.useEffect.loadAndValidateCart"]));
                        const validated = sanitized.map({
                            "CartProvider.useEffect.loadAndValidateCart.validated": (it)=>{
                                const prod = productMap.get(Number(it.id));
                                if (!prod) return null; // removed product
                                if (!prod.is_tersedia) return null; // not available
                                // Keep the freshest product data (price, name, foto_url) but preserve qty/catatan
                                return {
                                    ...prod,
                                    qty: it.qty,
                                    catatan: it.catatan
                        ...
                        }["CartProvider.useEffect.loadAndValidateCart.validated"]).filter(Boolean);
                        if (validated.length) {
                            // Instead of restoring silently, place validated items into pending state.
                            // The user must explicitly restore them to the active cart to avoid surprises.
                            setPendingCart(validated);
                        } else {
                            localStorage.removeItem('maindihatta_cart');
                        }
                    } catch (err) {
                        // On fetch failure, fallback to sanitized saved cart (but still pending)
                        console.warn('Could not validate cart with server, storing pending items', err?.message || err);
                        setPendingCart(sanitized);
                    }
                }
            }["CartProvider.useEffect.loadAndValidateCart"];
            loadAndValidateCart();
            return ({
                "CartProvider.useEffect": ()=>controller.abort()
            })["CartProvider.useEffect"];
        }
    }["CartProvider.useEffect"], []);
    const restorePendingCart = ()=>{
        ...
        setCart(pendingCart);
        localStorage.setItem('maindihatta_cart', JSON.stringify(pendingCart));
        setPendingCart(null);
        __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$sonner$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toast"].success(`${pendingCart.length} item dipulihkan ke keranjang.`);
    };
    const discardPendingCart = ()=>{
        setPendingCart(null);
        localStorage.removeItem('maindihatta_cart');
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$sonner$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toast"])('Item tersimpan telah dihapus.', {
            duration: 3000
        });
    };
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "CartProvider.useEffect": ()=>{
            localStorage.setItem('maindihatta_cart', JSON.stringify(cart));
        }
    }["CartProvider.useEffect"], [
        cart
    ]);
    const addToCart = (product, qty = 1, catatan = '')=>{
        setCart((prev)=>{
        ...
        __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$sonner$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toast"].success(`${qty}x ${product.nama_produk} masuk keranjang!`);
    };
    ...
    const totalPrice = cart.reduce((sum, item)=>sum + Number(item.harga_jual) * item.qty, 0);
...
_s(CartProvider, "JZVLZXUJ0sAiCVRFFe/n321qyYo=");
_c = CartProvider;
function useCart() {
    _s1();
...
//# sourceMappingURL=_20b0761d._.js.map